...
from config import (DEFAULT_COLS, DIVERGING_SCALE, GRADIENT_CMAP,
                    HEATMAP_SCALE, MAX_HEADS_PER_ROW)
from processing.attention import (diff_map, inject_bfactor, normalise,
                                  residue_labels, to_mean_scores)
...
def render(s: AppState) -> None:
    ...
    st.caption("Averaged over all layers, heads, and query positions from the raw tensor.")

    if not s.npy_files:
        st.info("Point the sidebar to a CAAT output directory containing .npy files.")
        return

    chosen = st.selectbox("Attention tensor (.npy)", s.npy_files,
                          format_func=lambda f: f.name, key="mean_npy_sel")
    ...
    with col_bar:
        ...
        fig.update_layout(
            ...
            xaxis_title="Residue", yaxis_title="Score",
            height=380, margin=dict(t=40, b=10),
        )
        st.plotly_chart(fig, use_container_width=True)

    with col_line:
        fig2 = px.line(x=labels, y=scores,
                       labels={"x": "Residue", "y": "Score"},
                       title="Score Profile")
        fig2.add_hline(y=cutoff, line_dash="dash", line_color="red",
...
def render_heads(s: AppState) -> None:
    ...
    chosen = st.selectbox("Attention tensor (.npy)", s.npy_files,
                          format_func=lambda f: f.name, key="head_npy_sel")
    arr = loader.npy(chosen)
    ...
    with st.expander(f"🔲 All {H} heads — Layer {layer_i}"):
        ...
        for h in range(H):
            mini = px.imshow(
                arr[layer_i, h],
                color_continuous_scale=HEATMAP_SCALE,
                title=f"Head {h}", aspect="auto",
            )
            mini.update_layout(height=180, margin=dict(t=28, b=4, l=4, r=4),
...
def render_diff(s: AppState) -> None:
    ...
    st.caption("Select two tensors. The dashboard computes the residue×residue delta "
    ...
    q_file = col_q.selectbox("Query .npy",  s.npy_files,
                              format_func=lambda f: f.name, key="diff_q_sel")
    t_file = col_t.selectbox("Target .npy", s.npy_files,
                              index=min(1, len(s.npy_files) - 1),
                              format_func=lambda f: f.name, key="diff_t_sel")
    ...
    fig2.update_layout(
        ...
        xaxis_title="Residue", yaxis_title="Avg Δ Score",
...
def render_compare(s: AppState) -> None:
    ...
    fig.update_layout(
        ...
        xaxis_title="Residue Index", yaxis_title="Mean Score",
    ...
    if len(selected) >= 2:
        ...
        fig2.update_layout(
            ...
            xaxis_title="Residue Index", yaxis_title="Δ Score",
            height=320,
        )
        st.plotly_chart(fig2, use_container_width=True)
...
def render_gallery(s: AppState) -> None:
    ...
    kw = st.text_input("Filter by filename",
                       placeholder="mean  /  diff  /  protein name…",
                       key="gallery_filter")
    filtered = (
        [f for f in s.img_files if kw.lower() in f.name.lower()]
        if kw else s.img_files
    )
...
def render_3d(s: AppState) -> None:
    ...
    chosen_pdb = col_p.selectbox("PDB file", s.pdb_files,
                                  format_func=lambda f: f.name, key="3d_pdb_sel")

    score_options = ["None (color by spectrum)"] + [f.name for f in s.npy_files]
    chosen_score_name = col_s.selectbox("Attention tensor (.npy) — optional",
                                        score_options, key="3d_npy_sel")
    ...
    if chosen_score_name == "None (color by spectrum)":
        ...
        st.caption("Spectrum coloring. Select a .npy file to color residues by attention score.")
    else:
