...
df = pd.read_csv('all_rocklin_updated_TM.csv')
ddg_all = df["ddG"]
...
#decile_labels = pd.qcut(attention, q=10, labels=False, duplicates="drop")  # 0–9
decile_labels = pd.qcut(pd.Series(attention).rank(method="first"), 
                        q=10, labels=False)
records = []
records2 = []
for dec in range(10):
    vals = confidence[decile_labels == dec]
    ...
    ci_low, ci_high = stats.bootstrap((vals,), np.median, confidence_level=0.95,
                                       random_state=42, method='percentile').confidence_interval
...
ax.scatter(decile_labels + 1 + jitter, confidence, s=3, alpha=0.07,
...
r, p = stats.pearsonr(attention, confidence)
...
ax.set_ylabel("Change in \npLDDT (ΔpLDDT)", fontsize=14)
ax.set_xticks(range(1, 11))
ax.set_xlim(0.4, 10.7)
#ax.set_title("Stability is not reflected in final predictions", fontsize=11, fontweight="bold")
...
plt.tight_layout()
plt.savefig("S2_plddt.pdf", dpi=300, bbox_inches="tight")
