...
class ProteinMorph:
    ...
    def align_structures(self):
        ...
        for i, structure in enumerate(self.structures[1:], start=1):
            mobile_atoms = [
                atom for atom in structure.get_atoms() if atom.get_name() == "CA"
            ]
    ...
    def generate_morph(self, frames_between=10, output_dir="morph_frames"):
        ...
        for i in range(len(self.aligned_structures) - 1):
            ...
            frames_to_save = (
                interpolated
                if i == len(self.aligned_structures) - 2
                else interpolated[:-1]
            )
    ...
    def create_pymol_script(
        self, output_dir="morph_output", script_name="view_morph.pml"
    ):
    ...
    def create_animation(
        ...
        (line,) = ax.plot(
            [], [], [], "o-", markersize=3, linewidth=1, color="blue", alpha=0.6
        )
        ...
        def update(frame_idx):
            ...
            source_name = (
                self.frame_source_names[frame_idx]
                if hasattr(self, "frame_source_names")
                else ""
            )
            frame_text.set_text(
    ...
    def create_rotating_animation(
        ...
        def update(frame_idx):
            ...
            frame_text.set_text(
                f"Frame: {(frame_idx % len(frames_data)) + 1}/{len(frames_data)}"
            )
            return line, frame_text
...
if __name__ == "__main__":
    argparser = argparse.ArgumentParser(
        description="Visualize protein structure morphology through interpolation."
    )
    argparser.add_argument(
        "--structures", type=str, required=True, help="Folder containing PDB files."
    )
    argparser.add_argument(
        "--frames",
        type=int,
    ...
    argparser.add_argument(
        "--create-video", action="store_true", help="Create MP4 video animation."
    )
    argparser.add_argument(
        "--create-gif", action="store_true", help="Create GIF animation."
    )
    argparser.add_argument(
        "--rotating", action="store_true", help="Create rotating view animation."
    )
    argparser.add_argument(
        "--fps",
        type=int,
